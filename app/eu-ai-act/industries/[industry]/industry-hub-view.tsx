...
export const INDUSTRY_HUB_PAGE_SIZE = 24;

type IndustryHubViewProps = {
  industrySlug: string;
  pageNumber: number;
};
...
export function IndustryHubView({ industrySlug, pageNumber }: IndustryHubViewProps) {
  const industry = getHighIntentIndustry(industrySlug);
  if (!industry) {
    notFound();
  }

  const pages = getHighIntentPagesByIndustry(industrySlug).sort((a, b) =>
    a.path.localeCompare(b.path)
  );

  const totalPages = Math.max(1, Math.ceil(pages.length / INDUSTRY_HUB_PAGE_SIZE));

  if (pageNumber < 1 || pageNumber > totalPages) {
    notFound();
  }

  const paged = paginate(pages, pageNumber, INDUSTRY_HUB_PAGE_SIZE);
  const totals = getHighIntentTotals();
  const currentPath =
    pageNumber === 1
      ? `/eu-ai-act/industries/${industry.slug}`
      : `/eu-ai-act/industries/${industry.slug}/page/${pageNumber}`;
    ...
    <div className="min-h-screen bg-[#f6f4ef] pb-16">
      ...
      <SeoBreadcrumbJsonLd
        items={[
          ...
          { name: 'Industries', href: '/eu-ai-act' },
          { name: industry.name, href: currentPath }
        ]}
      />
      ...
      <header className="border-b border-border/60 bg-white/90">
        <div className="mx-auto w-full max-w-6xl px-6 py-10">
          <p className="text-xs uppercase tracking-[0.2em] text-muted-foreground">
            Industry high-intent hub
          </p>
          <h1 className="font-heading text-3xl font-semibold text-foreground">
            {industry.name} EU AI Act pages
          </h1>
          <p className="mt-2 text-sm text-muted-foreground">
            {pages.length} indexable pages for deployer/provider buyers across
            countries, intents, and artifacts.
          </p>
      ...
      <main className="mx-auto flex w-full max-w-6xl flex-col gap-8 px-6 pt-10">
        <Card className="border border-border/70 bg-white/90 p-6">
          <p className="text-sm text-muted-foreground">
            This industry hub contributes to the 3,888-page architecture ({totals.total}
            total pages) with static crawl paths.
          </p>
            ...
            <div className="flex flex-wrap gap-2">
              {Array.from({ length: totalPages }).map((_, index) => {
                ...
                const href =
                  page === 1
                    ? `/eu-ai-act/industries/${industry.slug}`
                    : `/eu-ai-act/industries/${industry.slug}/page/${page}`;
                return (
                  <Link
                    key={href}
                    href={href}
